...
| `--container-rid=<RUNTIME IDENTIFIER>` | The runtime identifier to build the container image for when calling the `PublishContainer` target. | Inferred from `RuntimeIdentifier` or `RuntimeIdentifiers` build property. | Any valid [.NET runtime identifier](https://learn.microsoft.com/en-us/dotnet/core/rid-catalog) operating system. |
